import { FACTORY_ADDRESS } from '@danielvindax/metaviral-sdk'
  ...
  const usersQuery = gql`
    query userCount($since: ISO8601DateTime, $till: ISO8601DateTime) {
      ethereum(network: bsc) {
        dexTrades(exchangeName: { in: ["MetaViral", "MetaViral v2"] }, date: { since: $since, till: $till }) {
          count(uniq: senders)
        }
      }
    }
  ...
  try {
    const result = await infoServerClient.request(gql`
      query tvl {
        ...
        token(id: "0xd30854a284C72Ef4969B74b64628DB4bD697e900") {
          derivedUSD
        }
      }
    `)
