    ...
    <Flex flexDirection="column" maxWidth="736px">
      ...
      <HowToCard number={1} title={t('Get Ready')}>
        <Text fontSize="14px" color="textSubtle">
          {t('Set up your')}{' '}
          {account ? (
            <StyledLink href={`${nftsBaseUrl}/profile/${account.toLowerCase()}`}>{t('MetaViral Profile')}</StyledLink>
          ) : (
            t('MetaViral Profile')
          )}
          {', '}
      ...
      <HowToCard number={2} title={t('Battle Time')}>
        <Text fontSize="14px" color="textSubtle">
          {t(
            'Trade DAR/MTV, MTN/MTV, and MTN/MUSD during the battle period to raise both your and your team’s score. See the Rules section below for more.',
          )}
        </Text>
      </HowToCard>
