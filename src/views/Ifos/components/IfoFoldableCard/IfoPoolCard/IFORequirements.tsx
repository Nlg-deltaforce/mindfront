...
import { Text, Flex, AccountIcon, TeamBattleIcon, Box, useTooltip, LinkExternal } from '@metaviralswap/uikit'
...
const NotOkNFT = ({ admissionProfile }) => {
  ...
  const keyword = '%MetaViral Squad NFT%'

  const rawText = t(`Set %MetaViral Squad NFT% as MetaViral Profile avatar`)
      ...
      element={
        <>
          <LinkExternal
            ...
            href={`https://metaviralswap.com/nfts/collections/${admissionProfile}`}
          >
            {t('MetaViral Squad NFT')}
          </LinkExternal>
          <br />
        </>
      }
...
const NotOkProfilePoints = ({ pointThreshold }) => {
  ...
  const keyword = '%MetaViral Profile%'

  const rawText = t(`Reach %point% or more %MetaViral Profile% points`, { point: pointThreshold })
    ...
    <TransWithElement
      ...
      element={
        <>
          ...
          <LinkExternal
            style={{ display: 'inline' }}
            href={`https://metaviralswap.com/nfts/profile/${account}/achievements/`}
          >
            {t('MetaViral Profile')}
          </LinkExternal>
        </>
