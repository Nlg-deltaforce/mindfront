...
import { Button, useModal } from '@metaviralswap/uikit'
...
const ContributeButton: React.FC<Props> = ({ poolId, ifo, publicIfoData, walletIfoData }) => {
  ...
  const handleContributeSuccess = async (amount: BigNumber, txHash: string) => {
    ...
    toastSuccess(
      ...
      <ToastDescriptionWithTx txHash={txHash}>
        {t('You have contributed %amount% MTN to this IFO!', {
  ...
  return (
    <Button
      ...
      disabled={isDisabled}
    >
      {isMaxCommitted && publicIfoData.status === 'live' ? t('Max. Committed') : t('Commit MTN')}
    </Button>
  )
}
