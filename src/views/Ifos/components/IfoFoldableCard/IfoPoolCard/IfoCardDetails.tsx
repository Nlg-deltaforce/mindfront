...
import { Text, Flex, Box, Skeleton, TooltipText, useTooltip } from '@metaviralswap/uikit'
...
const MaxTokenEntry = ({ maxToken, ifo, poolId }: { maxToken: number; ifo: Ifo; poolId: PoolIds }) => {
  const isCurrencyCake = ifo.currency === tokens.mtn
  ...
  const basicTooltipContent =
    ifo.version === 3.1
      ? t(
          'For the private sale, each eligible participant will be able to commit any amount of MTN up to the maximum commit limit, which is published along with the IFO voting proposal.',
        )
      : t(
          'For the basic sale, Max MTN entry is capped by minimum between your average MTN balance in the IFO MTN pool, or the pool’s hard cap. To increase the max entry, Stake more MTN into the IFO MTN pool',
        )

  const unlimitedToolipContent =
    ifo.version === 3.1
      ? t(
          'For the public sale, Max MTN entry is capped by your average MTN balance in the IFO MTN pool. To increase the max entry, Stake more MTN into the IFO MTN pool',
        )
      : t(
          'For the unlimited sale, Max MTN entry is capped by your average MTN balance in the IFO MTN pool. To increase the max entry, Stake more MTN into the IFO MTN pool',
        )

  const tooltipContent = poolId === PoolIds.poolBasic ? basicTooltipContent : unlimitedToolipContent

  const { targetRef, tooltip, tooltipVisible } = useTooltip(tooltipContent, { placement: 'bottom-start' })
  const label = isCurrencyCake ? t('Max. MTN entry') : t('Max. token entry')
...
const IfoCardDetails: React.FC<IfoCardDetailsProps> = ({ isEligible, poolId, ifo, publicIfoData, walletIfoData }) => {
  ...
  // unlimited pool just show the mtv left
    ...
    if (status === 'coming_soon') {
      return (
        <>
          ...
          {ifo[poolId].cakeToBurn !== '$0' && <FooterEntry label={t('MTN to burn:')} value={ifo[poolId].cakeToBurn} />}
          <FooterEntry
