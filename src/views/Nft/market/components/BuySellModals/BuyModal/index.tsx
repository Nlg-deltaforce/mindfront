...
import { InjectedModalProps } from '@metaviralswap/uikit'
...
import { ChainId } from '@danielvindax/metaviral-sdk'
...
const wbnbAddress =
  CHAIN_ID === String(ChainId.MAINNET) ? tokens.wmtv.address : '0x094616f0bdfb0b526bd735bf66eca0ad254ca81f'

const BuyModal: React.FC<BuyModalProps> = ({ nftToBuy, onDismiss }) => {
  const [stage, setStage] = useState(BuyingStage.REVIEW)
  const [confirmedTxHash, setConfirmedTxHash] = useState('')
  const [paymentCurrency, setPaymentCurrency] = useState<PaymentCurrency>(PaymentCurrency.MTV)
  ...
  const walletBalance = paymentCurrency === PaymentCurrency.MTV ? formattedBnbBalance : formattedWbnbBalance
  const walletFetchStatus = paymentCurrency === PaymentCurrency.MTV ? bnbFetchStatus : wbnbFetchStatus

  const notEnoughBnbForPurchase =
    paymentCurrency === PaymentCurrency.MTV
  ...
  useEffect(() => {
    if (bnbBalance.lt(nftPriceWei) && wbnbBalance.gte(ethersToBigNumber(nftPriceWei)) && !isPaymentCurrentInitialized) {
      setPaymentCurrency(PaymentCurrency.WMTV)
  ...
  const continueToNextStage = () => {
    if (paymentCurrency === PaymentCurrency.WMTV && !isApproved) {
