...
import { CardBody, Flex, Text, CardRibbon } from '@metaviralswap/uikit'
...
const PoolCard: React.FC<{ pool: DeserializedPool; account: string }> = ({ pool, account }) => {
  ...
  const isCakePool = earningToken.symbol === 'MTN' && stakingToken.symbol === 'MTN'
    ...
    >
      <PoolCardHeader isStaking={accountHasStakedBalance} isFinished={isFinished && sousId !== 0}>
        <PoolCardHeaderTitle
          ...
          subTitle={isCakePool ? t('Earn MTN, stake MTN') : t('Stake %symbol%', { symbol: stakingToken.symbol })}
        />
