...
import { Box, Text, Flex, MessageText, Message } from '@metaviralswap/uikit'
  ...
  return (
    <>
      <Box>
        ...
        <LightGreyCard>
          <BalanceRow title={t('Mtn to be locked')} value={lockedAmount} newValue={newLockedAmount} decimals={2} />
          <BalanceRow
            title="apy"
            unit="%"
      ...
      {showLockWarning && (
        <Box mt="16px" maxWidth="370px">
          <Message variant="warning">
            <MessageText>
              {t('You will be able to withdraw the staked MTN and profit only when the staking position is unlocked')}
            </MessageText>
          </Message>
        </Box>
      )}
