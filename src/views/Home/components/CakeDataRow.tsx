import { Flex, Heading, Skeleton, Text } from '@metaviralswap/uikit'
 ...
 * https://planetfinanceio.medium.com/metaviralswap-works-with-planet-to-help-cake-holders-f0d253b435af
 * https://twitter.com/metaviralswap/status/1523913527626702849
...
const CakeDataRow = () => {
  ...
  const {
    ...
    async () => {
      const totalSupplyCall = { address: tokens.mtn.address, name: 'totalSupply' }
      const burnedTokenCall = {
        address: tokens.mtn.address,
        name: 'balanceOf',
  ...
  return (
    <Grid>
      ...
      <StyledColumn noDesktopBorder style={{ gridArea: 'a' }}>
        <Text color="#EFAC00" mb="10px">{t('Total supply')}</Text>
        {cakeSupply ? (
          <Balance decimals={0} lineHeight="1.1" fontSize="24px" color="#000000" bold value={cakeSupply} />
        ) : (
          <>
            <div ref={observerRef} />
            <Skeleton height={24} width={126} my="4px" />
      ...
      <StyledColumn noDesktopBorder style={{ gridArea: 'b' }}>
        <Text color="#EFAC00" mb="10px">{t('Market cap')}</Text>
        {mcap?.gt(0) && mcapString ? (
          <Text fontSize="24px" color="#000000">{t('$%marketCap%', { marketCap: mcapString })}</Text>
        ) : (
          <Skeleton height={24} width={126} my="4px" />
        )}
      </StyledColumn>
      <StyledColumn noDesktopBorder style={{ gridArea: 'c' }}>
        <Text color="#EFAC00" mb="10px">{t('Burned to date')}</Text>
        {burnedBalance ? (
          <Balance decimals={0} lineHeight="1.1" fontSize="24px" color="#000000" bold value={burnedBalance} />
        ) : (
          <Skeleton height={24} width={126} my="4px" />
        )}
      </StyledColumn>
      <StyledColumn noDesktopBorder style={{ gridArea: 'd' }}>
        <Text color="#EFAC00" mb="10px">{t('Current emissions')}</Text>

        <Text fontSize="24px" color="#000000">{t('%cakeEmissions%/block', { cakeEmissions: emissionsPerBlock })}</Text>
      </StyledColumn>
    </Grid>
  )
}
