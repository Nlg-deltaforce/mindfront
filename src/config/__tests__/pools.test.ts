...
describe('Config pools', () => {
  ...
  it.each(pools.map((pool) => [pool.sousId, pool.contractAddress]))(
    ...
    (sousId, contractAddress) => {
      const duplicates = pools.filter((p) => contractAddress[10434] === p.contractAddress[10434])
      expect(duplicates).toHaveLength(1)
    },
  )
  it.each(poolsToTest.filter((pool) => pool.earningToken.symbol !== 'MTV'))(
  ...
  it.each(poolsToTest.filter((pool) => pool.stakingToken.symbol !== 'MTV'))(
