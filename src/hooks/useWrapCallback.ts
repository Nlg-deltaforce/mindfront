import { Currency, currencyEquals, ETHER, WETH } from '@danielvindax/metaviral-sdk'
      ...
      return {
        ...
        execute:
          sufficientBalance && inputAmount
            ? async () => {
                try {
                  ...
                  addTransaction(txReceipt, {
                    summary: `Wrap ${inputAmount.toSignificant(6)} MTV to WMTV`,
                    type: 'wrap',
                  })
                } catch (error) {
            ...
            : undefined,
        inputError: sufficientBalance ? undefined : t('Insufficient MTV balance'),
      }
    }
          ...
          sufficientBalance && inputAmount
            ? async () => {
                try {
                  ...
                  addTransaction(txReceipt, { summary: `Unwrap ${inputAmount.toSignificant(6)} WMTV to MTV` })
                } catch (error) {
                  console.error('Could not withdraw', error)
                }
              }
            : undefined,
        inputError: sufficientBalance ? undefined : t('Insufficient WMTV balance'),
      }
    }
