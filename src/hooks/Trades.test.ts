...
import { Pair, TokenAmount, CurrencyAmount, Trade } from '@danielvindax/metaviral-sdk'
...
describe('Trade', () => {
  describe('#useAllCommonPairs', () => {
    ...
    it('should filter only exist Pair', () => {
      mockUsePairs.mockReturnValue([
        [
          PairState.EXISTS,
          new Pair(new TokenAmount(mainnetTokens.wmtv, '1'), new TokenAmount(mainnetTokens.mtn, '1')),
        ],
        [
          PairState.INVALID,
          new Pair(new TokenAmount(mainnetTokens.musd, '1'), new TokenAmount(mainnetTokens.mtn, '1')),
        ],
        [
          PairState.LOADING,
          new Pair(new TokenAmount(mainnetTokens.musd, '1'), new TokenAmount(mainnetTokens.wmtv, '1')),
        ],
        [PairState.EXISTS, null],
      ])

      const { result } = renderHook(() => {
        const pairs = Trades.useAllCommonPairs(mainnetTokens.wmtv, mainnetTokens.mtn)
        return {
          pairs,
        }
      })
    ...
    it('should filter out duplicated Pair', () => {
      mockUsePairs.mockReturnValue([
        ...
        [
          PairState.EXISTS,
          new Pair(new TokenAmount(mainnetTokens.mtn, '1'), new TokenAmount(mainnetTokens.wmtv, '1')),
        ],
        [PairState.EXISTS, null],
      ])
    ...
    it('should get all pair combinations wmtv, mtn', () => {
      mockUsePairs.mockClear()
      renderHook(() => {
        Trades.useAllCommonPairs(mainnetTokens.wmtv, mainnetTokens.mtn)
      })

      expect(mockUsePairs).toMatchSnapshot()
    })

    it('should get all pair combinations, wmtv, wmtv', () => {
      mockUsePairs.mockClear()
      renderHook(() => {
        Trades.useAllCommonPairs(mainnetTokens.wmtv, mainnetTokens.wmtv)
      })
  ...
  describe('#useTradeExactIn/Out', () => {
    ...
    it('should call with maxHops 1 with singleHopOnly', () => {
      const allowPairs = [new Pair(new TokenAmount(mainnetTokens.wmtv, '1'), new TokenAmount(mainnetTokens.mtn, '1'))]
      const argA = CurrencyAmount.ether('1000000')
      const argB = mainnetTokens.mtn
      renderHook(
        () => {
