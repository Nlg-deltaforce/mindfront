import { Currency, currencyEquals, JSBI, Price } from '@danielvindax/metaviral-sdk'
...
export default function useBUSDPrice(currency?: Currency): Price | undefined {
  ...
  const tokenPairs: [Currency | undefined, Currency | undefined][] = useMemo(
    () => [
      ...
      [wrapped?.equals(musd) ? undefined : wrapped, musd],
      [chainId ? WBNB : undefined, musd],
    ],
  ...
  return useMemo(() => {
    ...
    if (wrapped.equals(WBNB)) {
      if (busdPair) {
        ...
        return new Price(currency, musd, price.denominator, price.numerator)
      }
      return undefined
    }
    // handle busd
    if (wrapped.equals(musd)) {
      return new Price(musd, musd, '1', '1')
    }
    ...
    if (busdPairState === PairState.EXISTS && busdPair && busdPair.reserveOf(musd).greaterThan(bnbPairBNBBUSDValue)) {
      const price = busdPair.priceOf(wrapped)
      return new Price(currency, musd, price.denominator, price.numerator)
    }
    if (bnbPairState === PairState.EXISTS && bnbPair && busdBnbPairState === PairState.EXISTS && busdBnbPair) {
      if (busdBnbPair.reserveOf(musd).greaterThan('0') && bnbPair.reserveOf(WBNB).greaterThan('0')) {
        const bnbBusdPrice = busdBnbPair.priceOf(musd)
        const currencyBnbPrice = bnbPair.priceOf(WBNB)
        const busdPrice = bnbBusdPrice.multiply(currencyBnbPrice).invert()
        return new Price(currency, musd, busdPrice.denominator, busdPrice.numerator)
      }
    }
...
export const useCakeBusdPrice = (): Price | undefined => {
  const cakeBusdPrice = useBUSDPrice(tokens.mtn)
...
export const useBNBBusdPrice = (): Price | undefined => {
  const bnbBusdPrice = useBUSDPrice(tokens.wmtv)
  return bnbBusdPrice
}
