...
describe('hooks', () => {
  describe('#queryParametersToSwapState', () => {
    test('MTV to DAI', () => {
      expect(
        queryParametersToSwapState(
          parse(
            'inputCurrency=MTV&outputCurrency=0x6b175474e89094c44da98b954eedeac495271d0f&exactAmount=20.5&exactField=outPUT',
          ),
        ),
      ).toEqual({
        [Field.OUTPUT]: { currencyId: '0x6B175474E89094C44Da98b954EedeAC495271d0F' },
        [Field.INPUT]: { currencyId: 'MTV' },
        typedValue: '20.5',
    ...
    test('should return MTV MUSD pair by default', () => {
    ...
    test('does not duplicate MTV for invalid output token', () => {
      expect(queryParametersToSwapState(parse('outputCurrency=invalid'))).toEqual({
        [Field.INPUT]: { currencyId: '' },
        [Field.OUTPUT]: { currencyId: 'MTV' },
        typedValue: '',
        independentField: Field.INPUT,
        pairDataById: {},
        derivedPairDataById: {},
    ...
    test('output MTV only', () => {
      expect(queryParametersToSwapState(parse('outputCurrency=mtv&exactAmount=20.5'))).toEqual({
        [Field.OUTPUT]: { currencyId: 'MTV' },
        [Field.INPUT]: { currencyId: '' },
        typedValue: '20.5',
        independentField: Field.INPUT,
    ...
    test('invalid recipient', () => {
      expect(queryParametersToSwapState(parse('outputCurrency=MTV&exactAmount=20.5&recipient=abc'))).toEqual({
        [Field.OUTPUT]: { currencyId: 'MTV' },
        [Field.INPUT]: { currencyId: '' },
        typedValue: '20.5',
        independentField: Field.INPUT,
    ...
    test('valid recipient', () => {
      expect(
        queryParametersToSwapState(
          parse('outputCurrency=MTV&exactAmount=20.5&recipient=0x0fF2D1eFd7A57B7562b2bf27F3f37899dB27F4a5'),
        ),
      ).toEqual({
        [Field.OUTPUT]: { currencyId: 'MTV' },
        [Field.INPUT]: { currencyId: '' },
