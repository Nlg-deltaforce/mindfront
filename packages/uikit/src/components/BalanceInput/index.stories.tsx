...
export const SiwtchUnits: React.FC = () => {
  const CAKE_PRICE = 69;
  const [editingUnit, setEditingUnit] = useState<"MTN" | "USD">("MTN");
  const conversionUnit = editingUnit === "MTN" ? "USD" : "MTN";
  ...
  const switchEditingUnits = () => {
    const editingUnitAfterChange = editingUnit === "MTN" ? "USD" : "MTN";
  ...
  const handleCakeChange = (input: string) => {
    ...
    if (editingUnit === "MTN") {
      ...
      setValues({
        MTN: Number.isNaN(inputAsFloat) ? "" : `${inputAsFloat / CAKE_PRICE}`,
        USD: input,
      });
    }
  };
